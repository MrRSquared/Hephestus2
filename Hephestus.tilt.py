...
print("We are about to find some blocks.")
...
try:
    while 1:
      ...
      # if the pixy finds a block, do stuff.
      if Count > 0:
        Frame = Frame + 1
        ...
        if Locked_On_Block:
          ...
          for Index in range (0, Count):
            if Blocks[Index].m_index == Locked_Block_Index:
              ...
              # Ensure that the drive stays at zero when not in range (or really close).
              if drive != 0:
                  speed = ((drive))
              else:
                  speed = 0
              leftSpeed =-speed - turnOutput #Turn output should be negative on both+  
              rightSpeed = speed - turnOutput #
              # Set the motor speed.
